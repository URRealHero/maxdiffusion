...
from ..pipelines.wan.wan_pipeline_2_2_dense import WanPipeline2_2_Dense


class WanCheckpointer2_2_Dense(WanCheckpointer):
  ...
  def load_diffusers_checkpoint(self):
    pipeline = WanPipeline2_2_Dense.from_pretrained(self.config)
    return pipeline

  def load_checkpoint(self, step=None) -> Tuple[WanPipeline2_2_Dense, Optional[dict], Optional[int]]:
    restored_checkpoint, step = self.load_wan_configs_from_orbax(step)
    opt_state = None
    if restored_checkpoint:
      max_logging.log("Loading WAN pipeline from checkpoint")
      pipeline = WanPipeline2_2_Dense.from_checkpoint(self.config, restored_checkpoint)
  ...
  def save_checkpoint(self, train_step, pipeline: WanPipeline2_2_Dense, train_states: dict):
