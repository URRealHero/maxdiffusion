...
from maxdiffusion.trainers.base_wan_trainer import BaseWanTrainer, TrainState
import tensorflow as tf


class Wan2_2Trainer(BaseWanTrainer):
  ...
  def load_dataset(self, mesh, pipeline=None, is_training=True):
    ...
    Returns:
        Data iterator
    """
    # Stages of training as described in the Wan 2.2 paper - https://arxiv.org/pdf/2503.20314
...
def sample_wan22_expert_timesteps(timestep_rng, batch_size, scheduler, config):
  num_train_timesteps = scheduler.config.num_train_timesteps
  boundary_timestep = config.boundary_ratio * num_train_timesteps
  u = jax.random.uniform(timestep_rng, (batch_size,), dtype=jnp.float32)

  expert = getattr(config, "wan22_train_expert", "high")
  if expert == "high":
    return boundary_timestep + u * (num_train_timesteps - boundary_timestep)
  if expert == "low":
    return u * boundary_timestep

  raise ValueError(f"Unsupported wan22_train_expert={expert}. Expected 'high' or 'low'.")
...
def step_optimizer(state, data, rng, scheduler_state, scheduler, config):
  ...
  def loss_fn(params):
    ...
    bsz = latents.shape[0]
    timesteps = sample_wan22_expert_timesteps(timestep_rng, bsz, scheduler, config)
  ...
  metrics = {
      "scalar": {
          ...
          "debug/timestep_min": timestep_min,
          "debug/timestep_max": timestep_max,
          "debug/timestep_mean": timestep_mean,
      },
      "scalars": {},
  }
