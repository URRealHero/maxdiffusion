...
def load_clip_assets(sid, gen_dir, raw_root, height, width):
  ...
  # GT subject masks (white=subject) for BOTH cond (context) and tgt windows, sampled by the
  # same concat indices and split at the boundary. With these we crop the subject by mask bbox
  # (DSC) and never need YOLO -> no ultralytics dependency.
  def _seg(name, n):
